...
def showGPA(numstd,numcour):
    ...
    for i in range(0,numstd):
        ...
        for j in range(0,numcour):
            avg+=studentlist[i].getMark(j)*courselist[j].getcredit()
            sumcr+=courselist[j].getcredit()
        result.append({'name':studentlist[i].getName(),'GPA':math.floor(avg/sumcr)})
...
def input_course(nc):
    for j in range(0,nc):
        ...
        courselist.append(course(Id,Name,cr))
                ...
                if len(courselist)>0:
                    ...
                    if (course_seletion<0) & (course_seletion>course_num):
                        ...
                        for k in range(0, student_num ):
                            studentlist[k].addMark(course_seletion,int(input(f"Mark for student {studentlist[k].getName()}: ")))
        elif option==6:
            print(" Student List ")
